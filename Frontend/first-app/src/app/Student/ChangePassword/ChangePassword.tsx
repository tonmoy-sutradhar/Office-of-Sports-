...
import { setPasswordlSchema, SetPasswordlSchema } from "@/app/Validation/LoggedInPassValid";
...
export default function SetPassword() {
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<SetPasswordlSchema>({
        resolver: zodResolver(setPasswordlSchema),
    });

    const setPassword = async (data: SetPasswordlSchema) => {
        try {
            ...
            await api.patch("/user/changePass", data, {
                headers: {
            ...
            setTimeout(() => {
                setSuccessMessage("");
                router.push("/Student/Profile"); // Redirect after 5 seconds
            }, 5000);
        } catch (error: any) {
            setCustomError(
            ...
            <div className="relative mb-6">
              ...
              {/* Toggle Visibility Button */}
                <button
                  ...
                  onClick={() => setShowoPassword((prev) => !prev)}
                   >
                    {showoPassword ? (
                     <VscEye
